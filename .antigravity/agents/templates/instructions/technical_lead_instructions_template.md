...
| Memory (live index — the archive is **not** read at spawn; see Project Memory below) | `.antigravity/agents/memory/Technical_Lead_Memory.md` |
...
Record durable facts in `.antigravity/agents/memory/Technical_Lead_Memory.md` (live index) with full fact bodies in `.antigravity/agents/memory/Technical_Lead_Memory_Archive.md` — **the archive is never read at spawn and never read in full**; open it only when an index line's keywords match the task at hand, locating the matching fact by grep. This role uses the two-tier split — rules and format: `.antigravity/agents/rules/Agent_Common_Read_On_Demand.md §8` (retrieval mechanics, when to open the archive) and `§1` (the underlying four-field fact shape, Troubleshooting Facts).
