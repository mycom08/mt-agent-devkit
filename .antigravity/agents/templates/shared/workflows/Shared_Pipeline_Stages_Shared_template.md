...
- **Strict mode:** read `.antigravity/agents/docs/stories/ST-XXXXXX.md` — subject to pre-flight only if the body contains a `## Reproduction` section (strict mode has no label mechanism; the section's presence is the bug-story marker — see `Story_Standard_PO.md §13`).
...
**If `Mode: strict`:** Read `.antigravity/agents/docs/stories/ST-XXXXXX.md` directly.
...
**Orchestrator pre-spawn: create the retro file skeleton** before spawning the implementer. Use the story ID and title from Stage 0. Write `.antigravity/agents/retros/ST-XXXXXX_retro.md`:
...
4. **Read the story:**
   ...
   - **Strict mode:** Agent reads `.antigravity/agents/docs/stories/ST-XXXXXX.md` directly
...
7. **CI/CD check:**
   - **GitHub mode:** if the story's Technical Scope includes any file under `.github/workflows/`, the implementer **must** follow `.antigravity/agents/rules/CICD_Validation_Guide.md` before opening a PR
...
10. **After implementation is ready for review** → update story status to `review`:
    ...
    - **Strict mode:** create `.antigravity/agents/docs/reviews/ST-XXXXXX_review.md` (see `Strict_Mode_Story_Guide.md` §Local Review Record); edit `**Status:** review` in the story MD; append comment entry to `## Comments` noting branch is ready for review
11. Agent writes retro section to `.antigravity/agents/retros/ST-XXXXXX_retro.md` per `Retro_Rules.md` before reporting back
...
4. **Resume the implementer** via `send_message` to `impl_session` (define and invoke new if expired). Pass:
...
1. Resolve **who to tag** following the lookup order in `.antigravity/agents/rules/Blocked_Request.md` § Step 1 — if no match is found, report back to the orchestrator to ask the user before proceeding
2. Fill in the template with the resolved name, confirmed items, missing items, and helpful commands
3. Write the filled-in content to `.antigravity/agents/tmp/blocked_<story-id>.md`
4. Record the block:
...
- `[skipped-step]` **Strict mode only:** Review-record MD created at `.antigravity/agents/docs/reviews/ST-XXXXXX_review.md`?
...
3. If changes are needed, resume Implementer via `impl_session` (define and invoke new if expired); re-run this fast path on completion
...
4. If AC issues found, resume Implementer via `impl_session` (define and invoke new if expired); re-run this fast path on completion
...
9. **If story AC passed** → QA updates automation coverage for the story then runs the full automation suite to check for regressions (see QA Rules §8–§9)
   - **If automation fails** → QA reports regression failures:
     ...
     → resume Implementer via `impl_session` to fix (define and invoke new if expired); on completion resume QA to revalidate (counts toward loop limit)
   - **If automation passes** → QA writes retro section to `.antigravity/agents/retros/ST-XXXXXX_retro.md` per `Retro_Rules.md` before reporting back; orchestrator executes the **Merge Procedure** below
...
4. PO writes retro section to `.antigravity/agents/retros/ST-XXXXXX_retro.md` per `Retro_Rules.md` before reporting back
...
Read `.antigravity/agents/retros/ST-XXXXXX_retro.md`. Check that all expected sections are populated (no remaining `*(pending)*` placeholders).
...
- `[skipped-step]` Ran `wc -c .antigravity/agents/working-record/*_Working_Record.md`; append one bullet per file over the `**Working record cap:**` value in `CLAUDE.md` (default 4,000 if absent), naming the file and its size.
- `[skipped-step]` Ran `wc -c .antigravity/agents/memory/*_Memory.md`; append one bullet per file over the `**Memory file cap:**` value in `CLAUDE.md` (default 10,000 if absent), naming the file and its size.
- `[skipped-step]` `Stage` and `Updated` refreshed in state file after this transition?
