...
The orchestrator acts as PO — read `.antigravity/agents/product_owner_instructions.md` and `.antigravity/agents/rules/Story_Standard_PO.md` before proceeding. **Do not spawn a PO agent.**
...
- Target sprint (if known). If the user says "next sprint" without a number, resolve it:
  ...
  - **Strict mode:** glob `.antigravity/agents/docs/stories/*.md`, collect all unique `**Sprint:**` field values, parse the trailing number from each (e.g. `sprint-3` → 3), take the highest, then add 1.
...
1. Read `.antigravity/agents/docs/story_counter.txt`, increment by 1, write back — derive the new `ST-XXXXXX` ID
...
3. Write `.antigravity/agents/docs/stories/ST-XXXXXX.md` using the format from `Strict_Mode_Story_Guide.md` §Story MD Format:
   - `**Status:** backlog`
...
4. Report created story file paths to the user (e.g., `.antigravity/agents/docs/stories/ST-000001.md`)
<!-- SHARED-END -->
