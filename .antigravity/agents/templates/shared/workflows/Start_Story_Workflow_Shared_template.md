...
**If `Mode: strict`** — read `**Status:**` field from `.antigravity/agents/docs/stories/ST-XXXXXX.md` and route using the same table above (status values are identical — see `Strict_Mode_Story_Guide.md` §Status Values).
...
- **Retro Review** — after Stage 5 (or immediately after Stage 4 if Stage 5 is disabled):
  1. Read `.antigravity/agents/retros/ST-XXXXXX_retro.md`
  ...
  4. Read `Sprint` from the state file; append a story section to `.antigravity/agents/retros/sprint_N_summary.md` (see Sprint_Workflow.md for format; create the file if it does not exist)
  5. Delete `.antigravity/agents/retros/ST-XXXXXX_retro.md`
