...
- **Non-feature sprint:**
  ...
  - **Strict mode:** Glob `.antigravity/agents/docs/stories/*.md`, collect all unique `**Sprint:**` field values, select the highest sprint number that has at least one story with `**Status:** backlog`. That sprint is the target.

**Story fetch:**
- **GitHub mode:** `gh issue list --label "sprint-N" --label "status:backlog" --state open`
- **Strict mode:** Glob `.antigravity/agents/docs/stories/*.md`, filter by `**Sprint:** sprint-N` AND `**Status:** backlog`

---

## Pipeline State

The orchestrator maintains `.antigravity/agents/tmp/refine_pipeline_state.md` to support resumption after unexpected termination and to preserve session IDs across stages.
...
3. **Spawn** one agent per role that has assigned stories (**model: Gemini Pro** for Developer/QA/Business Analyst; **model: Gemini Pro, effort: high** for Technical Lead); save session IDs to state file as `dev_session`, `tl_session`, `qa_session`, `ba_session`. If all stories belong to one role, spawn a single agent — no parallel spawn needed. If multiple roles are present, spawn them in parallel simultaneously.
...
2. **Spawn or resume** TL (**model: Gemini Pro, effort: high**) first (reuse `tl_session` if active); save/update `tl_session` in state file
3. TL answers all technical questions across all stories in a single pass and signals completion
4. **Then spawn or resume** PO (**model: Gemini Pro**) (reuse `po_session` if active); save/update `po_session` in state file — pass TL's answers summary so PO has full technical context before making scope decisions
...
3. For each story that had questions, the assigned agent reads all TL and PO replies. Before editing any story body, check whether TL or PO already made the edit — do not duplicate an update that was already applied.
   - **If all answers are clear** → agent records final clearance:
     - **GitHub mode:** post a final comment: "All open points resolved — story is ready for development. PO please move to ready." Set `**Thread Status:** Resolved`
     - **Strict mode:** append comment entry to story MD `## Comments`: `"All open points resolved — story ready for development."` Set `**Thread Status:** Resolved` within that comment entry
   - **If answers raise new questions** → agent appends follow-up to the same comment thread / `## Comments` section; reports back to orchestrator (loop to Stage 2)
...
6. Orchestrator deletes `.antigravity/agents/tmp/refine_pipeline_state.md`, then deletes any remaining files in `.antigravity/agents/tmp/` with `rm .antigravity/agents/tmp/*.md`
