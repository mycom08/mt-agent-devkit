...
```json
{
  "0.0.7": [".antigravity/agents/templates/..."],
  "0.0.8": {
    "files": [".antigravity/agents/templates/..."],
    "descriptions": {
      ".antigravity/agents/templates/workflows/Sync_Devkit_Workflow_template.md": "Added auto-approve, checksums, WebFetch fallback"
    },
    "checksums": {
      ".antigravity/agents/templates/workflows/Sync_Devkit_Workflow_template.md": "sha256:abc123"
    }
  }
}
```
...
**Source:** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/rules/{filename}_template.md`
**Target:** `.antigravity/agents/rules/{filename}.md`
...
Applies to: `Agent_Common.md`, `Audit_Rules.md`, `Blocked_Request.md`, `Business_Analyst_Rules.md`, `CICD_Validation_Guide.md`, `Clean_Code_Rules.md`, `Developer_Rules.md`, `Logging_Standard.md`, `Product_Owner_Rules.md`, `QA_Rules.md`, `Retro_Rules.md`, `Story_Standard.md`, `Story_Standard_Dev.md`, `Story_Standard_PO.md`, `Story_Standard_QA.md`, `Story_Standard_TL.md`, `Technical_Lead_Rules.md`, `UI_UX_Designer_Rules.md`, `UI_Prototype_Rules.md`, and `Strict_Mode_Story_Guide.md` (only if `Mode: strict`).

#### Workflow files — Overwrite

**Source (split — mode-specific):** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/{mode}/workflows/{filename}_template.md`
**Source (non-split):** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/workflows/{filename}_template.md`
**Target:** `.antigravity/agents/workflows/{filename}.md`

Read `**Mode:**` from this project's `CLAUDE.md` to resolve `{mode}` (`github` or `strict`).

For split candidates, fetch the mode-specific variant. Also fetch the referenced shared file at `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/shared/workflows/{name}_Shared_template.md`. Combine: shared `<!-- SHARED-START -->` / `<!-- SHARED-END -->` block first, then any non-comment content from the mode-specific variant. Write the combined result verbatim.
...
**Source:** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/scripts/check_devkit_version.ps1`
          `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/scripts/check_devkit_version.sh`
...
**Source:** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/instructions/{role}_instructions_template.md`
**Target:** `.antigravity/agents/instructions/{role}_instructions.md`
...
**Source:** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/shared/CLAUDE_Shared_template.md` (use the `<!-- SHARED-START -->` / `<!-- SHARED-END -->` block content)
...
**Source:** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/wiki/*_template.md`  
...
2. Write `LATEST_VERSION` to `.antigravity/agents/devkit_version.txt`
...
[If any [UPDATE REQUIRED] placeholders were inserted]:
  ...
  Search for [UPDATE REQUIRED] in .antigravity/agents/instructions/ to find them.
```
...
**1. Spawn one general-purpose subagent** (never a persistent role or session — fresh every run) with a fully self-contained prompt naming this project's own deployed `.antigravity/agents/rules/Audit_Rules.md` as the complete behavior spec:
...
.antigravity/agents/rules/Audit_Rules.md
...
a. Write them to `.antigravity/agents/internal/audit_report_YYYYMMDD_HHMMSS.md` (create `.antigravity/agents/internal/` if it does not exist):
   ```markdown
...
- **Stage 4's scan subagent is spawned fresh every run** — never a persistent role or session, and it reads only the files this run actually wrote via a model-generated merge strategy, never the full `.antigravity/agents/` tree
