...
1. Read `.antigravity/agents/docs/build_state.md` (relative to this project orchestrator folder).
...
- If phase is **not** `scaffold` → print the following error and stop:
  ...
  If you believe this is wrong, check .antigravity/agents/docs/build_state.md and correct the Phase field manually.
  ```
...
1. Print to the user:
   ...
   The scrum team is already set up in this folder. To begin sprint execution:
     ...
     1. Run: plan next sprint
        (uses the analysis docs in .antigravity/agents/docs/analysis/ as input)
...
- **Analysis docs path** — `<repo-path>/.antigravity/agents/docs/analysis/` (where Stage 4 placed the split docs)
- **CLAUDE.md path** — `<repo-path>/CLAUDE.md`
- **Sprint workflow path** — `<repo-path>/.antigravity/agents/workflows/Sprint_Workflow.md` (or equivalent for the installed mode)
...
- **Repo role:** <repo-role>
- **Analysis docs:** <absolute-repo-path>/.antigravity/agents/docs/analysis/
- **CLAUDE.md:** <absolute-repo-path>/CLAUDE.md
- **Sprint workflow:** <absolute-repo-path>/.antigravity/agents/workflows/Sprint_Workflow.md

## What to do

1. Read `<absolute-repo-path>/CLAUDE.md` to understand the repo's mode (github or strict) and available triggers.
2. Read `<absolute-repo-path>/.antigravity/agents/docs/analysis/` — this folder contains the split analysis documents for this repo (implementation roadmap, architecture, etc.).
3. Run `create stories` using the analysis docs as input:
   - Read `<absolute-repo-path>/.antigravity/agents/workflows/Create_Stories_Workflow.md` for instructions.
...
4. After stories are created, run `plan next sprint`:
   - Read `<absolute-repo-path>/.antigravity/agents/workflows/Plan_Sprint_Workflow.md` for instructions.
...
1. Read `.antigravity/agents/docs/build_state.md`.
