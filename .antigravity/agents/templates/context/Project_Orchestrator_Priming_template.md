...
- Its only workflow trigger is `build software`, which (once every repo is scaffolded) coordinates `create stories` + `plan next sprint` across all repos listed above. See `.antigravity/agents/workflows/Build_Software_Project_Workflow.md`.
- `.antigravity/agents/docs/build_state.md` tracks this folder's own build phase and the GitHub Project URL shared by every repo.
