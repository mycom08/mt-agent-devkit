...
**Previous:** 2.2 — Added §6 (Shell Command Rules — Permissions and Tool Choice), centralized here from five `Story_Standard_<role>_template.md` §15 restatements per devkit issue #130 (ST-000137); used the Dev/QA/TL/PO variants' PowerShell-preferred wording as canonical since it is the majority form on this surface — `Story_Standard_UIUX_template.md` §15 had drifted to the Claude-only "Always use Bash" wording and is corrected by this consolidation, not carried forward.
