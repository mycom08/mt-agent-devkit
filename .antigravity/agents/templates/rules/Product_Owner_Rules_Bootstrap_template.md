...
§1–§12 above are loaded at spawn. Nothing in `Product_Owner_Rules_Read_On_Demand.md` is. When a trigger below fires, fetch **only** the named section — locate it with grep, not the whole file.
