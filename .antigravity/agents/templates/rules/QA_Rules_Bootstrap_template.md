...
**Applies to:** QA agent
**Reference from:** `.antigravity/agents/qa_instructions.md`
...
2. **Read Story Standard (QA)** — `.antigravity/agents/rules/Story_Standard_QA.md`
3. **Read your Working Record** — `.antigravity/agents/working-record/QA_Working_Record.md`
4. **Read your Memory** — `.antigravity/agents/memory/QA_Memory.md`
...
If this repo has (or is paired with) a `-ui-prototype` companion repo, apply `.antigravity/agents/rules/UI_Prototype_Rules.md` before signing off any AC for a screen with a prototype counterpart.

**Logging review (source code stories only):**
When reviewing a story that added or changed log statements, verify compliance with `.antigravity/agents/rules/Logging_Standard.md` (correct level, single-log rule, full stack trace only at ERROR, no sensitive data) before signing off AC.
...
- Place all new documents in the correct feature-doc subfolder — see `.antigravity/agents/context/Document_Index.md` for paths
...
On any tooling/environment blocker (sandbox won't start, automation runner cannot connect, test scripts fail, CI/auth errors), follow the check-memory → fix → record-to-memory protocol in `.antigravity/agents/rules/Agent_Common_Read_On_Demand.md §2`.

---

## 14. On-Demand Rules — Routing Table

§1–§13 above are loaded at spawn. Nothing in `QA_Rules_Read_On_Demand.md` is. When a trigger below fires, fetch **only** the named section — locate it with grep, not the whole file.
