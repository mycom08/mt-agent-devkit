...
### Fact 13
- **Rule:** Orchestrator state files live in the orchestrating workspace's `.antigravity/agents/tmp/`, not the managed repo — gitignored in both modes, has a cleanup lifecycle, and must exist before the managed repo does. Bridge the two working dirs by recording the managed repo's absolute path plus per-step commit SHAs in the state file.
