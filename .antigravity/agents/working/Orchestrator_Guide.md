...
> Read by the orchestrator only — workflow routing, session management, and completion-report format for running the devkit's own AI Scrum team. No spawned subagent needs this file: each spawn receives its own instruction/rules/memory paths directly in its prompt. Content every role needs (Project Overview, Agent Roster, PR Approval Rule) lives in `AGENTS.md`.
...
> `.antigravity/agents/working/token-trace_sprint/<StoryID>_<RoleTag>_steps_done.md` (`RoleTag`: `dev`, `TL`,
...
step than a cold spawn. `AGENTS.md`'s resume-over-spawn rule depends on it; this field is what makes it
...
Read `.antigravity/agents/working/enhancement/Harness_Benchmark_Guide.md` and follow it. It covers baseline choice, story selection, the four kinds of state that survive a branch checkout and contaminate the second arm, the runbook, and the traps in reading the result.
...
| **strict** | No GitHub/MCP required. Local repo only. Stories and docs stored locally under `.antigravity/agents/` (gitignored). No pushes to remote — you control all merges. |

`init project` asks which mode you want. The choice is written to the generated `AGENTS.md` as `**Mode:** strict` or `**Mode:** github` and drives all workflow behavior in the target project.
...
2. init project <path/to/your/project>
   └─ Select: github
   └─ Injects: AGENTS.md, agent instructions, rules, memory files, working records

3. In your project — open Antigravity and use:
...
2. init project <path/to/your/project>
   └─ Select: strict
   └─ Injects: AGENTS.md (mode: strict), agent files (all gitignored)
   └─ Creates: .antigravity/agents/docs/ structure + story_counter.txt
...
```
<project>/
├── AGENTS.md                          ← Mode: github + sprint workflow commands
├── .gitignore                         ← .antigravity/agents/tmp/ + /result/ added
└── .antigravity/agents/
...
```
<project>/
├── AGENTS.md                          ← Mode: strict + sprint workflow commands
├── .gitignore                         ← .antigravity/agents/ (entire folder) + /result/ added
└── .antigravity/agents/                    ← entirely gitignored
    ├── context/Project_Priming_Bootstrap.md
    ├── instructions/                  ← 5 agent instruction files
...
Read `.antigravity/agents/workflows/Analyst_Workflow.md` for the complete pipeline before executing.
...
Read `.antigravity/agents/workflows/Update_Project_Workflow.md` for the complete pipeline before executing.
...
Pipeline state is stored in `.antigravity/agents/tmp/build_software_state.md`. Running `build software` when the state file exists automatically resumes from the last completed stage.

Read `.antigravity/agents/workflows/Build_Software_Workflow.md` for the complete pipeline before executing.
...
Read `.antigravity/agents/workflows/Audit_Agent_Files_Workflow.md` for the complete pipeline before executing.

---

## Sprint Workflows (Devkit)

The devkit runs its own sprint workflows using the AI Scrum team under `.antigravity/agents/working/`. Trigger routing:

| Trigger | File |
|---|---|
| `continue sprint` | `.antigravity/agents/working/workflows/Sprint_Workflow.md` |
| `start story ST-XXXXXX` | `.antigravity/agents/working/workflows/Start_Story_Workflow.md` |
| `refine sprint` | `.antigravity/agents/working/workflows/Refine_Sprint_Workflow.md` |
| `plan next sprint` | `.antigravity/agents/working/workflows/Plan_Sprint_Workflow.md` |
| `create stories` | `.antigravity/agents/working/workflows/Create_Stories_Workflow.md` |
| `resume story ST-XXXXXX` | `.antigravity/agents/working/workflows/Resume_Story_Workflow.md` |

> `workflow help` (without further context) shows the devkit command reference above, not the sprint guide. If the user asks for sprint workflow help, read `.antigravity/agents/working/workflows/Workflow_Guide.md` and present it.
