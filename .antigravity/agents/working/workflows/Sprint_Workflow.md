...
The orchestrator maintains `.antigravity/agents/working/tmp/sprint_pipeline_state.md` to support resumption after unexpected termination.
...
- **Sprint end** — when no more `status:ready` stories remain:
  1. **Batch Retro Review** — process each story's retro file one by one in story order. For each:
     a. Read `.antigravity/agents/working/retros/ST-XXXXXX_retro.md`
     ...
     c. Present collected items to the user as proposed improvements; for each approved item, apply the change targeting the right artifact:
        ...
        - `[instruction]` → agent instruction files (`.antigravity/agents/working/instructions/`)
        - `[workflow]` → workflow files (`.antigravity/agents/working/workflows/`)
        - `[failure]` → rules or guardrail files (`.antigravity/agents/working/rules/`)
     ...
     d. Append a story section to the sprint summary file. Read `Sprint` from the state file: `sprint-N` → `.antigravity/agents/working/retros/sprint_N_summary.md`. Create the file if it does not exist:
        ```markdown
     ...
     e. Delete `.antigravity/agents/working/retros/ST-XXXXXX_retro.md`
     ...
     c. **If yes:**
        i. Build the export file content per `community-retros/README.md` format. Use filename pattern `sprint-<N>_<YYYY-MM-DD>.md`. Sections with no items must include `- None.` to keep structure consistent.
           ...
           **Date:** <YYYY-MM-DD>
           ...
           - <item, or "None.">
           ```
        ii. Write the export file to `.antigravity/agents/working/retros/devkit_contribution_sprint_N.md`.
        iii. Run `gh auth status` to check authentication:
             - **Authenticated:** run:
               ...
               gh issue create --repo mycom08/mt-agent-devkit \
                 ...
                 --body-file .antigravity/agents/working/retros/devkit_contribution_sprint_N.md
               ```
               Report the Issue URL to the user. Delete the local export file.
             - **Not authenticated or `gh` unavailable:** inform the user that the export file has been written to `.antigravity/agents/working/retros/devkit_contribution_sprint_N.md`. Instruct them to open an Issue labeled `retro:contribution` on `mycom08/mt-agent-devkit` with the export file contents, so the `apply retros` workflow can find it.
