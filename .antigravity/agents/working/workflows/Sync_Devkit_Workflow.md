...
Triggered by: `"sync devkit"` or `"sync devkit --auto"` in a target project's AGENTS.md
...
Before starting, read from the target project's `AGENTS.md`:
...
1. Read `**Devkit version:**` from `AGENTS.md` → `CURRENT_VERSION`
...
Full scan triggered (N files)
  ...
  Files to merge:      instructions/ (6), AGENTS.md
...
1. Read `**Mode:**` from this project's `AGENTS.md`
...
**Source:** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/workflows/{filename}_template.md`
...
> **Intentional divergence (ST-000037):** the template gained a new "Stage 4 — Audit Pass (detect-only)" after Stage 3 — a scoped subagent scan of files this run wrote via a model-generated merge strategy, filing findings as a devkit-repo Issue labelled `audit:contribution`. That stage is deliberately **not** reproduced here. This mirror's own header note already establishes why: `sync devkit` never actually runs inside the devkit repo — the devkit is the source of the templates, not a consumer of them — so there is no real "run" for a Stage 4 audit pass to scope itself to, and no `.antigravity/agents/rules/Audit_Rules.md` deployed copy in this repo for a subagent prompt to name (the devkit's own audit is `Audit_Agent_Files_Workflow.md`, a different mechanism with a different scope). Only the rules-file enumeration bump above (`Audit_Rules.md` added to both lists) applies to this reference-only mirror.
...
**Source:** `{DEVKIT_SOURCE_URL}/.antigravity/agents/templates/CLAUDE_TEMPLATE.md`

1. Fetch the latest template
2. Read the existing local `AGENTS.md`
...
1. Update `**Devkit version:**` in `AGENTS.md` to `LATEST_VERSION`
