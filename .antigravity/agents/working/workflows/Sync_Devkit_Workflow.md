...
**Older installs may still hold a raw base URL** of the form `https://raw.githubusercontent.com/{owner}/{repo}/main`. Read `{owner}` and `{repo}` out of whichever shape is present and carry on; the `AGENTS.md — Merge` step in Stage 2 rewrites the field to the canonical repository form.
...
**Source:** `{DEVKIT_RAW_BASE}/.claude/agents/templates/workflows/{filename}_template.md`
...
**Source:** `{DEVKIT_RAW_BASE}/.claude/agents/templates/CLAUDE_TEMPLATE.md`
