...
**Triggers — always path-filtered.** Agent memory commits (`.antigravity/agents/memory/`), README/docs edits, and other non-code changes must not burn CI runs:

```yaml
on:
  push:
    branches: [main]
    paths-ignore: ['.antigravity/**', '**.md', 'docs/**']
  pull_request:
    paths-ignore: ['.antigravity/**', '**.md', 'docs/**']
```
