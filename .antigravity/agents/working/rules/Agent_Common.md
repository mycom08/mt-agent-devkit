...
1. Read `.antigravity/agents/working/rules/Retro_Rules.md` for the three questions and format
2. Open `.antigravity/agents/working/retros/ST-XXXXXX_retro.md` (story ID is in your spawn prompt)
...
- Never commit any file under `.antigravity/agents/working/` other than memory files
...
**File:** one per agent per story, `.antigravity/agents/working/token-trace_sprint/<StoryID>_<RoleTag>_steps_done.md` — `RoleTag` is `dev`, `TL`, `qa`, `po`, `ba`, or `uiux`. Never share a file across roles or stories. Gitignored — never commit.
