...
1. **GitHub assignees** — `gh issue view <number> --repo mycom08/mt-agent-devkit --json assignees --jq '[.assignees[].login] | join(", @")'`
...
2. Write the filled-in comment to `.antigravity/agents/working/tmp/blocked_<story-id>.md`
3. Post it: `gh issue comment <number> --repo mycom08/mt-agent-devkit --body-file .antigravity/agents/working/tmp/blocked_<story-id>.md`
4. Change the story label: `gh issue edit <number> --repo mycom08/mt-agent-devkit --remove-label "status:ready" --add-label "status:blocked"`
...
**Needed to unblock:** <what this enables, e.g. ST-000045 workflow update>
