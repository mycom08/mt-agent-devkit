...
> UI/UX Designer working rules for story work. This is your day-to-day reference. `.antigravity/agents/working/rules/Story_Standard.md` remains the full cross-role source for anything not covered here.
...
**Design Source** — an optional pointer-style field (wireframe link, backlog reference). Read it in Step 1 of `UI_UX_Designer_Rules_Bootstrap.md` §2 before building anything; it is the source of truth for which screens/flows to build. Do not invent screens or interactions the Design Source and AC do not call for.
...
- **Never use the `@` prefix** — write role names without it (e.g., `**PO**`, `**TL**`)
