...
**Reference from:** `.antigravity/agents/working/instructions/technical_lead_instructions.md`
...
2. **Read Story Standard (TL)** — `.antigravity/agents/working/rules/Story_Standard_TL.md`
3. **Read your Working Record** — `.antigravity/agents/working/working-record/Technical_Lead_Working_Record.md`
...
- **Source code / script changes only** — verify compliance with `.antigravity/agents/working/rules/Clean_Code_Rules.md` (meaningful names, single responsibility, no side effects, error handling) for `.sh` and `.ps1` scripts
...
- **UI Prototype PRs (only if the story touches a repo generated by Build Software's UI-bearing companion-repo convention):** apply `.antigravity/agents/working/rules/UI_Prototype_Rules.md` before approving. Not applicable to the devkit's own repo (markdown-only, no UI-bearing companion repos), but this mirror stays in sync with the template per Project_Priming §15.
