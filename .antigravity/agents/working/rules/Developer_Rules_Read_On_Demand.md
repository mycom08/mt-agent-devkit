...
**Read this file:** never as part of the Pre-Work Sequence, in whole or in part. Fetch **one section** when its trigger fires — the trigger table in `Developer_Rules_Bootstrap.md` §18 names which. Use the `read-section` skill (`.antigravity/skills/read-section/`) with `grep -nE "^## [0-9]+\."` to bound the extraction; do not read the whole file.
