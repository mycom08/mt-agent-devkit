...
Not loaded at spawn. When a trigger fires, fetch **only** that section from `.antigravity/agents/working/context/Project_Priming_Read_On_Demand.md` with the `read-section` skill.
...
**mt-agent-devkit** is a Antigravity devkit that scaffolds a complete AI Scrum team into any target project.
...
- ❌ Sprint workflow commands not yet wired into the devkit's own AGENTS.md
