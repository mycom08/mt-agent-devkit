...
| Rules | `.antigravity/agents/working/rules/Product_Owner_Rules.md` |
...
Record durable facts in `.antigravity/agents/working/memory/Product_Owner_Memory.md`. Rules and format: `.antigravity/agents/working/rules/Agent_Common.md §2` (PO records `## Stored Facts` only).
...
6. Write your retrospective section to `.antigravity/agents/working/retros/ST-XXXXXX_retro.md` — read `.antigravity/agents/working/rules/Agent_Common.md §4` for format; overwrite the `## Product Owner` section only
...
For each selected story, check if AC is clear and testable. If questions exist, create `.antigravity/agents/working/tmp/PO_questions.md` and report to the orchestrator.
...
5. Delete `.antigravity/agents/working/tmp/PO_questions.md` if it exists
...
Update `.antigravity/agents/working/working-record/Product_Owner_Working_Record.md` at start and end of each session per `.antigravity/agents/working/rules/Agent_Common.md §5`. Log Completed (story IDs, backlog prioritization, acceptance decisions, scope gating), In Progress, and Impediments.
