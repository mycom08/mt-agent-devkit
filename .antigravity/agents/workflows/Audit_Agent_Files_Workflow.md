...
**Path:** `.antigravity/agents/internal/audit_report_YYYYMMDD_HHMMSS.md` — timestamp, not a run ID, so staleness is self-evident from the filename alone. Only **one** report is ever in flight; `.antigravity/agents/internal/` is gitignored (never committed).
...
Before doing anything else, glob `.antigravity/agents/internal/audit_report_*.md`.
...
.antigravity/agents/working/rules/Audit_Rules.md
...
1. Orchestrator takes the subagent's returned finding block verbatim and writes it into a new report file at `.antigravity/agents/internal/audit_report_YYYYMMDD_HHMMSS.md` (create `.antigravity/agents/internal/` if it does not exist), with the header block filled in (`version.txt` contents, scanned file count from the subagent, `**Status:** pending-approval`).
