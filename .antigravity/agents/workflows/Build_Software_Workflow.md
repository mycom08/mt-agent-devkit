...
Spawn **two general-purpose agents** (**model: Gemini Pro**) in the same message:
...
8. **Project Priming Context (orchestrator-direct, no agent).** Read `.claude/agents/templates/context/Project_Root_Priming_template.md` and write it to the project orchestrator folder as `.antigravity/agents/context/Project_Priming.md`, substituting:
   - `{{PROJECT_NAME}}` → product name from user's idea
...
1. Spawn **one general-purpose agent** (**model: Gemini Pro**) with a fully self-contained inline prompt (the agent has no memory of this conversation):
...
1. Spawn **one general-purpose agent** (**model: Gemini Pro**, not the devkit's own UI/UX Designer agent — same rationale as Java Skeleton Generation staying general-purpose: the devkit's own role agents assume sprint story context) with a fully self-contained inline prompt (the agent has no memory of this conversation):
...
1. Spawn **one general-purpose agent** (**model: Gemini Pro**, not the devkit's own role agents — same rationale as Java Skeleton/UI Prototype Generation staying general-purpose) with a fully self-contained inline prompt (the agent has no memory of this conversation):
