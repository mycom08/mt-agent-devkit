...
Triggered by: `"init project"` or `"init project [path]"` in AGENTS.md
...
The orchestrator maintains `.antigravity/agents/tmp/init_project_state.md` to support resumption.
...
4. **Ask the user about AI Provider and mode:**
   > "Which AI Provider will this project use? Reply `claude` (default) or `antigravity`.
   > 
   > Do you want **strict mode** for this project?
   >
   > **Strict mode** — no GitHub, MCP, or CI required. Stories and all agent docs are stored locally under `.antigravity/agents/` (gitignored). Branches and commits use your project's external story IDs if available. No pushes to remote — you control merges.
   >
   > **GitHub mode** (default) — full GitHub Issues, PRs, and Actions integration.
   >
   > Reply with your provider and mode (default: claude, github)."

   Store the answers as `Provider: claude` or `Provider: antigravity` and `Mode: strict` or `Mode: github` in the state file.
5. Write the state file with `Stage: 0`, the resolved `TARGET_PROJECT` path, the selected provider, and the selected mode.
...
| Existing AGENTS.md | Check if `AGENTS.md` exists at `TARGET_PROJECT` root |
...
- If `.antigravity/agents/` already exists in the target project → warn the user: list which files already exist and ask: **"Agent files already exist in this project. Overwrite all / Skip existing / Cancel?"**
...
- If `.antigravity/agents/` does not exist → proceed normally
...
| `templates/instructions/*_instructions_template.md` (×6) | `.antigravity/agents/[role]_instructions.md` | Adaptive |
| `templates/rules/*_template.md` (×25) | `.antigravity/agents/rules/[name].md` | 12 Mechanical, 13 Adaptive — see the Rules files section below for exactly which |
| `templates/{mode}/workflows/*_template.md` (×8 split) + `templates/workflows/*_Workflow_template.md` (×2 non-split) | `.antigravity/agents/workflows/[name].md` | Mechanical (all 10) |
...
If `README.md` exists at `TARGET_PROJECT` root → do not overwrite it. Instead append an `## AI Scrum Team` section (with a `---` separator) pointing to `AGENTS.md`, `.antigravity/agents/context/Project_Priming.md`, and the `workflow help` command — same append-not-replace handling as `AGENTS.md`.
If `README.md` does not exist → generate a full README from the template.

Adapt the template:
- Replace `{{PROJECT_NAME}}` with the detected project name
- Replace `{{PROJECT_DESCRIPTION}}` with the same 1–2 sentence description used for `AGENTS.md`
...
| What | Path |
|---|---|
| Stories | `.antigravity/agents/docs/stories/ST-XXXXXX.md` |
| Review Records | `.antigravity/agents/docs/reviews/ST-XXXXXX_review.md` |
| Story Counter | `.antigravity/agents/docs/story_counter.txt` |

---

#### `.antigravity/agents/context/Project_Priming.md`
...
**Target:** `.antigravity/agents/[role]_instructions.md` (strip `_template` suffix; files go at the root of `.antigravity/agents/`, not in a subdirectory)
...
- **13 of 25 rules files need real adaptation**, not just token substitution — confirmed by diffing real generated repos, not by scanning for `{}` tokens (several of these have zero bracketed placeholders and still needed rewriting):
  - `Developer_Rules_Bootstrap.md` — adapt §5 (testing & verification commands) to detected tooling; its pair `Developer_Rules_Read_On_Demand.md` carries no tooling-specific content but rides along in this tier (not yet byte-diff-verified as safe for the mechanical script)
  - `QA_Rules_Bootstrap.md` — adapt §4 (testing rules) to detected test framework; its pair `QA_Rules_Read_On_Demand.md` also needs the same `{test-command}`/`{integration-test-command}` substitution in its Pre-PR Gate table (rare — QA-as-Implementer only)
  - `Technical_Lead_Rules_Bootstrap.md` — adapt §4 (design standards) to detected tech stack; its pair `Technical_Lead_Rules_Read_On_Demand.md` carries no tooling-specific content but rides along in this tier (not yet byte-diff-verified as safe for the mechanical script)
  ...
  - `Logging_Standard.md` — the four-level semantics, single-log rule, and sensitive-data prohibition are copied verbatim; only §4 (Log Format) needs real adaptation — replace or supplement the default Java pattern-layout example with the target project's actual tech-stack logging convention (e.g., structured JSON, Python `logging`, Go `zap`/`zerolog`)
...
> `Mode: strict`'s blanket `.antigravity/agents/` entry already covers `.antigravity/agents/internal/` (and every other agent-infrastructure path) — no separate line is added for strict mode.
...
2. For `AGENTS.md`: specify whether it will be **appended to** or **created new**.
...
2. Review AGENTS.md and confirm: Mode: strict
...
- **AGENTS.md and README.md are additive** — never overwrite an existing file of either in full; always append the orchestrator block
...
- **Preserve template structure in Adaptive tier** — when generating adaptive files (like `Project_Priming.md`), do not delete standard structural sections (like Glossary or Workflow) just because some placeholders cannot be filled. Replace the `{placeholders}` with project facts where possible, and retain the document's baseline structure exactly as it appears in the template.
- **Comprehensive placeholder substitution** — when adapting instruction and rules files, ensure that EVERY project-specific placeholder (e.g. `{test-command}`, `{github-org}/{repo-name}`, `{language}`) is successfully identified and replaced across all generated files. Never leave unresolved `{placeholder}` brackets behind in the final output unless they are explicit runtime agent variables (like `Sprint_{N+1}`).
- **Mandatory README integration** — the orchestrator MUST ensure `README.md` is updated to point to the `.antigravity/` folder as described. If `README.md` already exists, you MUST append the `## AI Scrum Team` section. Do not skip this step, as agents rely on the README to locate their priming context.
