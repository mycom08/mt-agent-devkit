...
| `templates/rules/*_template.md` (×20) | `.antigravity/agents/rules/[name].md` | 10 Mechanical, 10 Adaptive — see the Rules files section below for exactly which |
...
- **11 of 21 rules files verbatim** (`{github-org}/{repo-name}` substituted, nothing else): `Agent_Common_Bootstrap`, `Agent_Common_Read_On_Demand`, `Audit_Rules`, `Blocked_Request`, `CICD_Validation_Guide`, `Clean_Code_Rules`, `Product_Owner_Rules`, `Retro_Rules`, `Story_Standard_TL`, `Strict_Mode_Story_Guide`, `UI_Prototype_Rules`
...
- **10 of 21 rules files need real adaptation**, not just token substitution — confirmed by diffing real generated repos, not by scanning for `{}` tokens (several of these have zero bracketed placeholders and still needed rewriting):
