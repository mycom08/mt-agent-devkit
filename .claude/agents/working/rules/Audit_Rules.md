...
**Applies to:** the `audit agent files` workflow's scan subagent (`.claude/agents/workflows/Audit_Agent_Files_Workflow.md`).
**Scope:** devkit-internal corpus audit only — `.claude/agents/templates/**`, `.claude/agents/workflows/**`, `.claude/agents/working/**` (excluding this file's own findings-output directory, `.claude/agents/internal/`).
