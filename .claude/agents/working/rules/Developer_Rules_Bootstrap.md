...
§1–§6 above are loaded at spawn. Nothing in `Developer_Rules_Read_On_Demand.md` is. When a trigger below fires, fetch **only** the named section with the `read-section` skill (`.claude/skills/read-section/`, bound on `^## [0-9]+\.`) — not the whole file.
