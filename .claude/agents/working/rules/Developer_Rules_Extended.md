...
**Applies to:** Developer agent — devkit's own team only (`.claude/agents/working/`). Relocated out of `Developer_Rules.md` and `developer_instructions.md` 2026-08-20, applying the pattern from issue #123 to the devkit's own agent team first (the distributable `.claude/agents/templates/` role rules are unchanged — a separate story). `Developer_Rules.md` and `developer_instructions.md` are each read in full on every Dev spawn regardless of story content; the four sections below apply only in scenarios that don't arise on most stories (a mid-implementation ambiguity, a live user override, Dev acting as reviewer instead of implementer, a sprint-refinement task the orchestrator explicitly assigns). Read this file **only when the matching scenario actually occurs** — do not read it as part of the standard Pre-Work Sequence. `Developer_Rules.md` §2/§11 and `developer_instructions.md`'s Refine Sprint Task heading each still carry a one-line pointer to their relocated section here.
...
**Version:** 1.0 (created 2026-08-20, split out of `Developer_Rules.md` v1.5 and `developer_instructions.md` per devkit issue #123).
