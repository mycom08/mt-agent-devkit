...
- **If `on.pull_request` exists with no branch/path restriction narrower than the story's changed paths** (the common case): **skip the `ci-validation` push entirely.** Open the PR as normal — the `pull_request`-triggered run that fires when the PR opens **is** the pre-merge validation run. Cite that run's URL in the PR's **CI Validation** section. Pushing to `ci-validation` first would produce a second, identical run against the same commit with no added signal.
- **If not** (the workflow is `push`-only, or the `pull_request` trigger has a `branches`/`paths` filter that would exclude this story's changes): proceed with Steps 1–3 below.
...
**Previous:** 1.3 — New exception added: a new check's first red run genuinely catching a defect is an expected, acceptable gate outcome  
**Created:** 2026-06-16
