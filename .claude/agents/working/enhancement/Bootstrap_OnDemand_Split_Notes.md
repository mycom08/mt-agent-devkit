...
- **TL/PO/QA's split predates this axis and hasn't been redone — TL and QA done, PO still open.** They each had a `*_Rules_Extended.md`, but it was built under issue #123's older, narrower test — *"does this apply to a rare scenario?"* (e.g. TL/QA acting as story implementer) — which silently assumes every spawn is a story spawn and so never questions story pre-start / PR-gate / git-workflow content. **TL** (2026-08-24, see "TL rename pass" below) and **QA** (2026-08-24, see "QA rename pass" below) were each verified against the new axis and found to already satisfy it — their existing splits isolate only genuinely rare scenarios (TL: acting as implementer, context anchoring; QA: acting as implementer, post-Done hotfix); both were renamed to `_Bootstrap.md`/`_Read_On_Demand.md`, given a routing table, and their instructions files now flag every on-demand trigger explicitly ("otherwise skip"), matching Dev's/PO's pattern. **PO remains unrenamed and unflagged** — same gap, not yet addressed. None of the three have been benchmarked (only Developer spawns were measured — `Bench_2026-08-21_Findings.md`, `Bench4_2026-08-24_Findings.md`); the TL and QA passes made no token-savings claim for exactly this reason. **Redo PO's split (rename + instructions-level flagging) the same way before treating the team as consistent, or before porting the pattern to another project's harness.**
