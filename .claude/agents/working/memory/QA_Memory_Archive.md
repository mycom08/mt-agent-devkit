...
> Full-text archive for `QA_Memory.md`'s Keyword Index tier — devkit-internal two-tier memory pilot (`Agent_Common.md §12`, issue #118). Not read every spawn; open only when an index line's keywords match your current task. To find one fact: `grep -n "^### Fact " QA_Memory_Archive.md` to list every heading with its line number, then bounded-read from that fact's line to the next fact's line minus one (or to end of file for the last fact) — never a full-file read.
