...
### Fact 13
- **Rule:** `Agent_Common.md` rules restated in the five per-role rules files (Developer/QA/Technical Lead/Product Owner/UI-UX Designer) come in two flavors: true bare pointers (Troubleshooting Protocol, Stage-Transition Commit — one line + a `§N` citation) and full restatements that then *also* cite `Agent_Common` for the extra detail (the retention rule was this, before ST-000131). The paying-twice tell is a rule that both spells out the substance **and** ends with a parenthetical "(see `Agent_Common.md §N` for …)" — that's a restatement pretending to be a pointer. Fix by cutting to the one-line-action + bare-`§N`-citation shape the other two already use. Also: the working mirror's `Agent_Common.md` and the template's `Agent_Common_template.md` have **different section numbers** for the same content (mirror consolidated Pre-Work Sequence + Working Record into one §1; template keeps them separate, Working Record is its own §5) — every one of the 5 role files' working copy cites `§1` and its template copy cites `§5` for the identical rule. This is the Project_Priming §15 "intentionally-diverged mirror" carve-out, not a bug; don't force them to match.
- **Applies when:** editing any rule in the 5 role rules files that restates something `Agent_Common.md`/`Agent_Common_template.md` already owns.
- **Evidence:** ST-000131 (issue #131) — retention rule was restated in full in all 5 role files (working mirror + template = 10 files), reduced to bare pointers; confirmed the §1 (mirror) vs §5 (template) split while editing.
- **Expires when:** the two `Agent_Common` copies converge on one section numbering, or restated content becomes generated rather than hand-authored.
