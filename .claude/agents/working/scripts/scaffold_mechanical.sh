...
for role in Business_Analyst Developer Product_Owner QA Technical_Lead UI_UX_Designer; do
  ...
  if [[ "$is_two_tier" == true ]]; then
    ...
    printf '# %s Memory Archive\n\n> Full-text archive for `%s_Memory.md` Keyword Index tier — see `Agent_Common_Read_On_Demand.md §8`. Not read every spawn; open only when an index line keyword matches your current task, using the `read-section` skill (`.claude/skills/read-section/`, heading marker `^### Fact `) — never a full-file read.\n\n## Stored Facts\n\n*(none yet)*\n' \
