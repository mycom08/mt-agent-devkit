...
- **Sprint end** — when no more `status:ready` stories remain:
  ...
  3. **Cleanup** — delete the state file, then delete any remaining files in `.claude/agents/working/tmp/` with `rm .claude/agents/working/tmp/*.md`. Agents must also delete any tmp files they created immediately after the file is no longer needed (e.g., after `gh` call using `--body-file`).
