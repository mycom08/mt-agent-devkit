...
Update at **start of session** and **end of session**, using rewrite-in-place snapshot semantics (see `Agent_Common.md §1`). Keep the **3 most recent story entries** — the retention unit is story entries, not calendar days — capped at ≤ 10,000 characters (`wc -c`), not a line count.
