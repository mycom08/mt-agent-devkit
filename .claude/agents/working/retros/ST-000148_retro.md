...
- `[failure]` Reinforces the existing `[skip ci]`-keys-on-the-path-filter rule, which is bootstrap-tier and was read this session — and still missed. The rule is stated per *push*, but the decision that matters is per *PR head commit*: a bookkeeping commit whose own files are outside the filter still suppresses the whole PR's check run when it lands on top of content that is inside it. The result was an empty rollup, caught only because the checks command was run explicitly rather than assumed. Suggest restating the rule at the level of "the PR's head commit", not "this push's files". The same shape bit twice: a later bookkeeping-only push, this time *without* `[skip ci]`, still emptied the rollup — a fresh head SHA fires the path filter, the filter does not match, no run is created, and check runs attach to a SHA, so the earlier passing run no longer counts. The durable fix is ordering, not phrasing: on a PR whose content sits under a path filter, the head commit must always be one that matches that filter — land bookkeeping commits first, or re-touch a filtered file last.
