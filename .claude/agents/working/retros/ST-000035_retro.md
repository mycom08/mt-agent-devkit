...
## QA
### Impediments & Unclear Points
- `[failure]` Running the Layer-1 differential gate directly in the primary working directory (instead of matched `git worktree` checkouts on both sides) produced a wrong baseline — 53 `[ERROR]` lines instead of the true 67 — because gitignored runtime files left on disk from prior sessions (`working-record/*.md`) are read by `_resolve_file_ref`'s repo-verbatim root and are absent from a fresh worktree. Corrected in QA_Memory Fact 2: worktree-vs-worktree is now required for every differential run, not only a working-tree-disturbance convenience.
- None otherwise — all 18 AC were fully pre-resolved by the refinement thread and the live scope-revision comment; zero ambiguity remained at validation time.

### Process Suggestions
- `[workflow]` A story whose own acceptance gate is a differential tool-run count should have QA (not only the reviewer) re-run it from matched worktrees rather than trusting either side's reported number — this is the second role in this story to catch the same class of measurement error (reviewer round 2 corrected round 1's "~70" to "67 `[ERROR]`"; QA then found a directory-dependent undercount hiding inside that same command). Reinforces the TL retro's own suggestion above about differential gates.

### What Worked Well
- Confirming a local checkout is behind its remote by diffing the specific commits (`git diff <local-HEAD> <PR-head> --stat`) before pulling avoided disturbing an intentionally-uncommitted retro file while still validating against PR-head-equivalent deliverable content.
- Tracing all three crash-recovery states as a closed enum with one row per value (rather than spot-checking a couple) surfaced that the design correctly avoids the recurring "unobservable terminal state falls through to the wrong branch" failure pattern this team has hit before.
- Independently re-deriving the "Mandatory Reading" heading-coverage nit (2 of 6 role Rules files) confirmed TL's non-blocking classification was correct — reduced coverage, not a false-positive risk — rather than accepting the classification on faith.
