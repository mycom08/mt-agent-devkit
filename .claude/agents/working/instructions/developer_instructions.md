...
Record durable facts in `.claude/agents/working/memory/Developer_Memory.md` (live index) with full fact bodies in `.claude/agents/working/memory/Developer_Memory_Archive.md`. This role uses the two-tier split — rules and format: `.claude/agents/working/rules/Agent_Common_Read_On_Demand.md §8` (retrieval mechanics, when to open the archive) and `§1` (the underlying four-field fact shape, Troubleshooting Facts).
