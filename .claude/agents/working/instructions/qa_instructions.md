...
Write your retro per `.claude/agents/working/rules/Agent_Common_Read_On_Demand.md §3`. Overwrite the `*(pending)*` placeholders in the `## QA` section only.

---

## Working Record

Update `.claude/agents/working/working-record/QA_Working_Record.md` at start and end of each session per `.claude/agents/working/rules/Agent_Common_Bootstrap.md §1`. Log Completed (validation findings, AC sign-offs, regression checks), In Progress, and Impediments.
