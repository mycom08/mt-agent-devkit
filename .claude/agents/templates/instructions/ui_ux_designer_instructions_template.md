...
Record durable facts in `.claude/agents/memory/UI_UX_Designer_Memory.md`. Rules and format (Stored Facts + Troubleshooting Facts): `.claude/agents/rules/Agent_Common.md §2`.
...
Write your retro per `.claude/agents/rules/Agent_Common.md §4`. Overwrite the `*(pending)*` placeholders in the `## Implementer — UI/UX Designer` section only.

---

## Working Record

Update `.claude/agents/working-record/UI_UX_Designer_Working_Record.md` at start and end of each session per `.claude/agents/rules/Agent_Common.md §5`. Log Completed (prototypes delivered, story IDs, PR/commit refs), In Progress, and Impediments.

---

## Version

**Version:** 1.0 — initial recorded version of the UI/UX Designer instructions template  
