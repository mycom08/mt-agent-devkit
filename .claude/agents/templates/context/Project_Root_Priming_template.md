...
- It has no `continue sprint`, `start story`, `plan next sprint`, or `refine sprint` triggers. Those live in each individual repo's own `{{ROOT_FILE}}` — open a session inside that repo folder to run them.
