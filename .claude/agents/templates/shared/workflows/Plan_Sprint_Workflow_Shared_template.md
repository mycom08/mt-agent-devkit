...
3. PO checks the active sprint:
   ...
   - **Non-feature sprint** (`feature_name: none`):
     ...
     - **Strict mode:** glob `{{AGENT_DIR_PREFIX}}/agents/docs/stories/*.md`, filter for `**Status:**` values of `in-progress`, `review`, or `testing` to find the active sprint; if none found, filter for `**Status:** done` and read the `**Sprint:**` field from the most recently created story (highest ID) — that is the current sprint N
...
7. **Roadmap reconciliation backstop** (feature sprint only; lightweight — this is the sprint-planning safety net for AC4, not the primary drain mechanism). If `feature_name` is set, glob `docs/feature/<feature_name>/plan/*Roadmap*.md`. If a roadmap file exists, for every story it defines, check whether a tracked issue/story already carries that story's `**Roadmap Source:** <roadmap-file> :: Phase N :: <story title>` marker (see `Product_Owner_Rules_Bootstrap.md §11a`). The check must confirm the marker exists **anywhere in the tracker, in any status** — a story that was correctly drained and has since moved past `status:backlog` (`ready`/`in-progress`/`done`) is still tracked, not drift:
   ...
   - **Strict mode:** grep `{{AGENT_DIR_PREFIX}}/agents/docs/stories/*.md` for each story's marker line using a whole-line match (e.g. `grep -Fxq "<marker>"` per file, not a plain substring grep, which has the same prefix-title false-positive risk) — no match means it's untracked.
...
2. PO locates planning inputs based on context:
   ...
   - **Non-feature sprint**:
     ...
     - **Strict mode:** glob `{{AGENT_DIR_PREFIX}}/agents/docs/stories/*.md`, filter by `**Status:** backlog`; sort by story ID (ascending)
...
5. **If questions remain** → PO creates `{{AGENT_DIR_PREFIX}}/agents/tmp/PO_questions.md` using the format below; proceed to Stage 3
...
7. **Workflow review (orchestrator):** check whether `{{AGENT_DIR_PREFIX}}/agents/tmp/plan_observations.md` exists
...
- **Observations** — whenever the orchestrator makes a judgment call not covered by this workflow, append a one-line bullet to `{{AGENT_DIR_PREFIX}}/agents/tmp/plan_observations.md` (create the file if it does not exist). These are reviewed at the end of Stage 4.
<!-- SHARED-END -->
