...
2. Check whether a tracked issue/story already carries this exact marker **before creating anything** — this check is what makes re-authoring or updating the same roadmap safe against duplicates; run it for every story on every write, not just the ones you think are new:
   - **Mode: github** — `gh issue list --repo {github-org}/{repo-name} --search "\"<marker from step 1>\" in:body" --state all --json number`. A returned match means this story is already drained — skip it.
   - **Mode: strict** — grep `.claude/agents/docs/stories/*.md` for the exact marker line. A match means this story is already drained — skip it.
