...
**Version:** 2.2 — Added §6 (Shell Command Rules — Permissions and Tool Choice), centralized here from five near-identical `Story_Standard_<role>_template.md` §15 restatements per devkit issue #130 (ST-000137); used the fullest of the divergent variants (958-char, PowerShell `.NET`-caveat + stale-file clause) as canonical, ported from the devkit-internal split's identical `working/rules/Agent_Common_Bootstrap.md` §6 (same story, working-tree side).
