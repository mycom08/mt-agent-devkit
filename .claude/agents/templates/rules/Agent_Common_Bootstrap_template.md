...
**Version:** 2.3 — §1's Working Record cap is now project-configurable via a `**Working record cap:**` field in `{{ROOT_FILE}}` (default 4,000, unchanged, if the field is absent), replacing the literal number that a `sync devkit` overwrite would silently re-impose over any local override — issue #128 (ST-000140).
