...
Triggered by: `"sync devkit"` or `"sync devkit --auto"` in the target project's {{ORCHESTRATOR_FILE}}
...
Before starting, read from this project's `{{ORCHESTRATOR_FILE}}`:
...
Full scan triggered (N files)
  ...
  Files to merge:      instructions/ (6), {{ORCHESTRATOR_FILE}}
...
1. Read `**Mode:**` from this project's `{{ORCHESTRATOR_FILE}}`
...
Read `**Mode:**` from this project's `{{ORCHESTRATOR_FILE}}` to resolve `{mode}` (`github` or `strict`).
...
**Target:** `{{AGENT_DIR_PREFIX}}/agents/Orchestrator_Guide.md`
...
**Process this file before the {{ORCHESTRATOR_FILE}} — Merge step below whenever both are in this run's update set.** The {{ORCHESTRATOR_FILE}} migration step (step 5 below) deletes local sections whose only surviving copy is this file — if the fetch/write here fails, skip the migration step entirely this run (leave the old sections in place) so a failed write never leaves the project with neither copy. Log the skip and retry both on the next sync.

#### {{ORCHESTRATOR_FILE}} — Merge

**Source:** `{DEVKIT_SOURCE_URL}/.claude/agents/templates/shared/Orchestrator_Shared_template.md` (use the `<!-- SHARED-START -->` / `<!-- SHARED-END -->` block content)
...
4. **Replace verbatim** from the updated template:
   ...
   - `## PR Approval Rule`
5. **Migrate a pre-split install (one-time, auto-split — only if the `Orchestrator_Guide.md — Overwrite` step above wrote successfully this run):** if the local `{{ORCHESTRATOR_FILE}}` still contains any of `## Agent Session Management`, `## Agent Completion Reports`, or `## Workflows` — this is an install from before ST-000043 — remove those sections (and only those three; `## Agent Roster` is never stripped, per step 3 above) from `{{ORCHESTRATOR_FILE}}` entirely and notify the user: _"Split the orchestrator-only sections out of {{ORCHESTRATOR_FILE}} into {{AGENT_DIR_PREFIX}}/agents/Orchestrator_Guide.md (new file) — this reduces the fixed context cost every spawned subagent pays."_
...
<scope list — the files this run actually wrote via rules/instructions/{{ORCHESTRATOR_FILE}} merge, from Stage 2's written-files log>
...
- **Stage 4 is detect-only and silent when clean** — it never writes to `rules/`, `instructions/`, or `{{ORCHESTRATOR_FILE}}`; a scoped run that finds nothing produces no output at all, only a filed Issue (or an instruction to file one manually) when it finds something
