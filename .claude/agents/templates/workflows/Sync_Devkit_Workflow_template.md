...
Triggered by: `"sync devkit"` or `"sync devkit --auto"` in the target project's {{ROOT_FILE}}
...
Before starting, read from this project's `{{ROOT_FILE}}`:
- `**Devkit source:**` — the devkit's GitHub repository URL (e.g. `https://github.com/YOUR_ORG/mt-agent-devkit`)
...
Fetch `{DEVKIT_RAW_BASE}/changes.json` to determine which files need updating.
...
Files to update (targeted):
  - {{ROOT_FILE}} (merge)
  - rules/Developer_Rules_Bootstrap.md (overwrite) — Added strict-mode gate    ← description shown when available
...
**Global Substitution Rule:** Before applying any of the merge strategies below, you must substitute framework placeholders in the fetched template content. Replace every instance of the `AGENT_DIR_PREFIX` placeholder (in double curly braces) with `{{AGENT_DIR_PREFIX}}`, the `ROOT_FILE` placeholder with `{{ROOT_FILE}}`, and the `AGENT_CLI_NAME` placeholder with `{{AGENT_CLI_NAME}}`. Do this for ALL files before considering them "verbatim" or merging them.


#### Rules files — Adapt to mode

**Source:** `{DEVKIT_RAW_BASE}/.claude/agents/templates/rules/{filename}_template.md`
**Target:** `{{AGENT_DIR_PREFIX}}/agents/rules/{filename}.md`
...
1. Read `**Mode:**` from this project's `{{ROOT_FILE}}`
...
Applies to: `Agent_Common_Bootstrap.md`, `Agent_Common_Read_On_Demand.md`, `Audit_Rules.md`, `Blocked_Request.md`, `Business_Analyst_Rules.md`, `CICD_Validation_Guide.md`, `Clean_Code_Rules.md`, `Developer_Rules_Bootstrap.md`, `Developer_Rules_Read_On_Demand.md`, `Logging_Standard.md`, `Product_Owner_Rules_Bootstrap.md`, `Product_Owner_Rules_Read_On_Demand.md`, `QA_Rules_Bootstrap.md`, `QA_Rules_Read_On_Demand.md`, `Retro_Rules.md`, `Story_Standard.md`, `Story_Standard_Dev.md`, `Story_Standard_PO.md`, `Story_Standard_QA.md`, `Story_Standard_TL.md`, `Technical_Lead_Rules_Bootstrap.md`, `Technical_Lead_Rules_Read_On_Demand.md`, `UI_UX_Designer_Rules.md`, `UI_Prototype_Rules.md`, and `Strict_Mode_Story_Guide.md` (only if `Mode: strict`).

#### Workflow files — Overwrite

**Source (split — mode-specific):** `{DEVKIT_RAW_BASE}/.claude/agents/templates/{mode}/workflows/{filename}_template.md`
**Source (non-split):** `{DEVKIT_RAW_BASE}/.claude/agents/templates/workflows/{filename}_template.md`
**Target:** `{{AGENT_DIR_PREFIX}}/agents/workflows/{filename}.md`

Read `**Mode:**` from this project's `{{ROOT_FILE}}` to resolve `{mode}` (`github` or `strict`).

For split candidates, fetch the mode-specific variant. Also fetch the referenced shared file at `{DEVKIT_RAW_BASE}/.claude/agents/templates/shared/workflows/{name}_Shared_template.md`. Combine: shared `<!-- SHARED-START -->` / `<!-- SHARED-END -->` block first, then any non-comment content from the mode-specific variant. Write the combined result verbatim.
...
- If missing → detect OS from the environment `sync devkit` is running in, then inject the matching hook JSON. If `{{AGENT_DIR_PREFIX}}/settings.json` already exists, merge this under its existing `hooks` key — do not remove existing hooks or keys. If it does not exist, create it with this content:
  - **Windows:**
    ```json
    { "hooks": { "SessionStart": [{ "matcher": "startup", "hooks": [{ "type": "command", "command": "powershell -File {{AGENT_DIR_PREFIX}}/agents/scripts/check_devkit_version.ps1", "timeout": 10 }] }] } }
    ```
  - **Mac/Linux:**
    ```json
    { "hooks": { "SessionStart": [{ "matcher": "startup", "hooks": [{ "type": "command", "command": "bash {{AGENT_DIR_PREFIX}}/agents/scripts/check_devkit_version.sh", "timeout": 10 }] }] } }
    ```

#### Instruction files — Merge

**Source:** `{DEVKIT_RAW_BASE}/.claude/agents/templates/instructions/{role}_instructions_template.md`
**Target:** `{{AGENT_DIR_PREFIX}}/agents/instructions/{role}_instructions.md`
...
**Source:** `{DEVKIT_RAW_BASE}/.claude/agents/templates/shared/orchestrator_instructions_shared_template.md` (use the `<!-- SHARED-START -->` / `<!-- SHARED-END -->` block content), combined with any non-comment content from `{DEVKIT_RAW_BASE}/.claude/agents/templates/{mode}/orchestrator_instructions_template.md` — same combine rule as split workflow files above.
...
Applies to: `orchestrator_instructions.md` (1 file).

**Rename migration (one-time):** after this file is written successfully, delete a leftover `Orchestrator_Guide.md` from that same directory if one exists — that is the pre-rename name of this same file, and leaving it in place gives the project two copies of the orchestrator reference that drift apart. Only delete it when the write above succeeded this run.

**Process this file before the {{ROOT_FILE}} — Merge step below whenever both are in this run's update set.** The {{ROOT_FILE}} migration step (step 5 below) deletes local sections whose only surviving copy is this file — if the fetch/write here fails, skip the migration step entirely this run (leave the old sections in place) so a failed write never leaves the project with neither copy. Log the skip and retry both on the next sync.

#### {{ROOT_FILE}} — Merge

**Source:** `{DEVKIT_RAW_BASE}/.claude/agents/templates/shared/Repo_Root_Shared_template.md` (use the `<!-- SHARED-START -->` / `<!-- SHARED-END -->` block content)
...
4. **Replace verbatim** from the updated template:
   ...
   - `## PR Approval Rule`
5. **Migrate a pre-split install (one-time, auto-split — only if the `orchestrator_instructions.md — Overwrite` step above wrote successfully this run):** if the local `{{ROOT_FILE}}` still contains any of `## Agent Session Management`, `## Agent Completion Reports`, or `## Workflows` — this is an install from before ST-000043 — remove those sections (and only those three; `## Agent Roster` is never stripped, per step 3 above) from `{{ROOT_FILE}}` entirely and notify the user: _"Split the orchestrator-only sections out of {{ROOT_FILE}} into {{AGENT_DIR_PREFIX}}/agents/orchestrator_instructions.md (new file) — this reduces the fixed context cost every spawned subagent pays."_
...
**Source:** `{DEVKIT_RAW_BASE}/.claude/agents/templates/wiki/*_template.md`  
...
[If any [UPDATE REQUIRED] placeholders were inserted]:
  ...
  Search for [UPDATE REQUIRED] in {{AGENT_DIR_PREFIX}}/agents/instructions/ to find them.
```
...
**1. Spawn one general-purpose subagent** (never a persistent role or session — fresh every run) with a fully self-contained prompt naming this project's own deployed `{{AGENT_DIR_PREFIX}}/agents/rules/Audit_Rules.md` as the complete behavior spec:
...
{{AGENT_DIR_PREFIX}}/agents/rules/Audit_Rules.md
...
<scope list — the files this run actually wrote via rules/instructions/{{ROOT_FILE}} merge, from Stage 2's written-files log>
...
a. Write them to `{{AGENT_DIR_PREFIX}}/agents/internal/audit_report_YYYYMMDD_HHMMSS.md` (create `{{AGENT_DIR_PREFIX}}/agents/internal/` if it does not exist):
   ```markdown
...
b. Run `gh auth status` to check authentication:
   - **Authenticated:** file an Issue on the devkit repository:
     ...
     gh issue create --repo mycom08/mt-agent-devkit \
       ...
       --body-file {{AGENT_DIR_PREFIX}}/agents/internal/audit_report_YYYYMMDD_HHMMSS.md
     ```
     Report the Issue URL to the user, then **delete the report file** — the Issue is now the record.
   - **Not authenticated or `gh` unavailable:** tell the user the report has been written to `{{AGENT_DIR_PREFIX}}/agents/internal/audit_report_YYYYMMDD_HHMMSS.md` and instruct them to open an Issue labeled `audit:contribution` on `mycom08/mt-agent-devkit` with the report's contents. Leave the report file in place in this case — it is the only record until the user files it manually.
...
- **Stage 4 is detect-only and silent when clean** — it never writes to `rules/`, `instructions/`, or `{{ROOT_FILE}}`; a scoped run that finds nothing produces no output at all, only a filed Issue (or an instruction to file one manually) when it finds something
- **Stage 4's scan subagent is spawned fresh every run** — never a persistent role or session, and it reads only the files this run actually wrote via a model-generated merge strategy, never the full `{{AGENT_DIR_PREFIX}}/agents/` tree
