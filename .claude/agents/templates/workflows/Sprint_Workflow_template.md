...
- **Sprint end** — when no more `status:ready` stories remain:
  ...
  2a. **Token Usage** — from `.claude/agents/retros/token_ledger.csv`, filter to this sprint's rows and append a `## Token Usage` subsection to the consolidated summary: total and average `subagent_tokens` per stage and per agent role, plus the spawn count (rows). If the ledger contains rows under a **different** `docs_sha` from a prior sprint, show the per-role/per-stage average delta (before → after, % change) and name both `docs_sha` values. Headline the per-spawn averages, not raw story totals — totals swing with story size. This measures subagent tokens only.
  3. **Cleanup** — delete the state file, then delete any remaining files in `.claude/agents/tmp/` with `rm .claude/agents/tmp/*.md`. Agents must also delete any tmp files they created immediately after the file is no longer needed (e.g., after `gh` call using `--body-file`). **Do not delete** `.claude/agents/retros/token_ledger.csv` or `.claude/agents/retros/token_baseline.md` — they are persistent historical records.
