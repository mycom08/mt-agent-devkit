...
2. TL reads `spec.md` + `business_requirements.md`:
   - Writes `architecture.md` covering: architecture choices, component design, data handling details, error handling strategies, CI/CD & deployment strategy, API-contract strategy, and any alternatives considered. Every diagram (Mermaid `.mmd` or PlantUML `.puml`) is written as a separate file under `diagrams/` and linked from `architecture.md` — never embedded inline.
     - **CI/CD & deployment strategy** covers: target environment(s) (e.g. AWS/GCP/self-hosted/undecided), how a merged PR gets from code to a running instance (deploy pipeline shape, not full IaC detail), and what "automation test" means for this project beyond unit tests (real integration/API test scope, not just "we'll add tests later"). This is a **decision to make explicit and hand to PO for roadmap stories**, not something the devkit scaffold already solves — Build Software's generated Dockerfile/docker-compose/CI only bootstrap a local-dev baseline (build, unit test, lint, a stub automation-test job) and never invent a real deploy target on their own.
     - If the spec doesn't name a deploy target (most idea descriptions won't), don't block on it: write a reasonable default assumption into `architecture.md` (e.g. "containerized service; deploy target not yet decided — local Docker Compose is the only environment until one is chosen") and log it as a `TL Question` only if the ambiguity is significant enough to change the architecture itself (e.g. serverless vs. containerized changes component design); otherwise just note it as an **Open Item** in `summary.md` later, same as any other deferred decision — don't interrogate the user over routine infra choices.
...
- **If `architecture.md`'s CI/CD & deployment strategy names anything beyond the devkit scaffold's local-dev baseline** (a real deploy target/pipeline, staging/prod environments, a real automation-test suite replacing the scaffold's stub job), include explicit story/stories for it in the roadmap (typically **Assigned: Technical Lead** for pipeline/environment setup, **Assigned: QA** for real automation-test suites) — don't leave it as an implicit assumption that scaffolding already covered it
