...
**Global Substitution Rule:** For every adaptive file generated in this section, always replace `{{AGENT_DIR_PREFIX}}` with `.claude`, `{{ORCHESTRATOR_FILE}}` with `CLAUDE.md`, and `{{AGENT_CLI_NAME}}` with `Claude Code`.
