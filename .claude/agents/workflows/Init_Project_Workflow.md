...
**Orchestrator_Guide.md generation:** Orchestrator-only content (workflow trigger table, agent roster, session management, completion-report format) lives in a separate devkit-merged file, not in `CLAUDE.md` — this keeps the fixed per-spawn context cost every subagent pays down to only what every role actually needs. It carries no project-specific placeholders, so it is written by `scaffold_mechanical.sh` (see the Mechanical tier below), not generated by an agent.
