...
**Universal substitutions:** For every adaptive file generated, always replace `{{AGENT_DIR_PREFIX}}` with `.claude`, `{{ORCHESTRATOR_FILE}}` with `CLAUDE.md`, and `{{AGENT_CLI_NAME}}` with `Claude Code`.
