...
7. **Git is the primary revert mechanism, not the validator verdict** — the validator comparison in step 6 is what *triggers* a revert decision, but the actual undo is `git checkout --`, scoped to exactly the files this batch touched (from step 5's diff --stat), never a wider `git reset`/`git clean`.
