---
name: read-section
description: Extract one numbered section (e.g. "## 5" or "### 15a") from a Markdown file cited as "<File>.md §N" or "<File>.md §Na" — without reading the whole file. Use this whenever a rule, instruction, or story comment cites a bare "§N" pointer with no attached extraction command.
---
...
`Read` has no heading-aware partial read — only a full read or a line
`offset`/`limit` — so a bare `§N` citation gives no way to compute a range on
its own. This skill is that recipe: two small commands, no whole-file read.

## Recipe

1. **List every heading at the cited level, with line numbers.**
   - Top-level section (`§N`, e.g. `§5`): `grep -n "^## " <file>`
   - Lettered subsection (`§Na`, e.g. `§15a`): `grep -n "^### " <file>`
2. **Find the boundary.** In that output, the line for `§N` is `start`. The
   next heading line *after* it is `end`. If `§N` is the last heading listed,
   there is no `end` — read to end of file instead.
...
a new citation site. The two-step recipe above only needs `N`; it derives the
boundary from the file itself and is exact.
