...
- [Cleanup] Trimmed the Developer instruction file, which every Developer spawn reads in full, from 5,630 to 2,393 bytes. The Refine Sprint Task procedure moved to `Developer_Rules_Read_On_Demand.md` §14 behind a one-line trigger pointer and a routing-table row; the pointer-only Project Memory and Working Record sections were removed, with the memory-archive path and its never-read-at-spawn warning folded into the Pre-Work Checklist table. Retrieval mechanics, working-record format, retention and cap were already carried by `Agent_Common_Read_On_Demand.md` §8 and `Agent_Common_Bootstrap.md` §1/§5.
- [Cleanup] Trimmed the Product Owner instruction file, which every PO spawn reads in full, from 9,915 to 2,869 bytes, catching the distributable templates up to the boundary already validated on the devkit's own team. The Story Closure, Refine Sprint, and Plan Next Sprint task procedures moved to new `Product_Owner_Rules_Read_On_Demand.md` §1–§3 behind one-line trigger pointers; the pointer-only Project Memory section was removed, folding its one unique detail (PO records `## Stored Facts` only) into the Pre-Work Checklist table's Memory row. Former §1 (Roadmap Story Drain) and §2 (Story Creation Template) renumbered to §4/§5, with `Product_Owner_Rules_Bootstrap.md` §13's routing table and `Story_Standard_PO.md`'s citation updated to match.
...
- [Cleanup] The `read-section` skill now ships a bundled `scripts/read_section.sh`: given a file plus a marker-regex and target-regex, it prints the section by line number and errors out instead of guessing when the target-regex is ambiguous. `SKILL.md` was trimmed to call the script directly, removing the hand-computed grep/sed line-range arithmetic that caused a measured over-read when the marker was narrowed to the target's own shape. `sync devkit`, `update project`, and the `-ui-prototype` scaffold set now fetch/expect the script alongside `SKILL.md`.
